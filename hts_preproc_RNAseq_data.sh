...
##This version searches for adapter contamination in both the R1 and R2 reads
...
do
  ...
  ##Quick pipeline overview
    ...
    ## 8  hts_QWindowTrim: remove poor quality sequence
    ## 9. hts_LengthFilter: use to remove all reads < 50bp
    ##10. hts_Stats: get stats on output reads
	...
	##The UC Davis course used Adapter Trimmer for RNAseq data, which is what is done here.
	...
	##I added -C because this does the following: "Check R2 as well as R1 (pe)"  Otherwise adapter contamination is left on the R2 reads.
	
	##hts_PolyATTrim trims poly A and T sequences from a read. The algorithm is borrowed from Fig 2, Bonfert et al. doi: 2017 10.1371/journal.pone.0170914  A sliding window of <window-size> (=6) is shifted from either end of the read (adjustable with --no-left and --no-right) until the <max-mismatch-errorDensity> is exceeded. The read is then trimmed as long as the following criteria are met:
  ...
  ##These settings may need to be adjusted depending on library type.
	...
	##-n [ --no-orphans ] Orphaned SE reads will NOT be written out
  ...
  call="hts_Stats -L ${outpath}/HTS_stats_log_files/${sample}_htsStats.log -N 'initial stats' -1 ${inpath}/${sample}*R1* -2 ${inpath}/${sample}*R2* | \
        ...
        hts_SeqScreener -s ${adapters} -C -k 15 -x .01 -A ${outpath}/HTS_stats_log_files/${sample}_htsStats.log -N 'screen remaining adapters' | \
        ...
        hts_LengthFilter -A ${outpath}/HTS_stats_log_files/${sample}_htsStats.log  -N 'remove reads < 50bp' -n -m 50 | \
